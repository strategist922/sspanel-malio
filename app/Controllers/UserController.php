...
class UserController extends BaseController
{
	public function index($request, $response, $args)
	{
		...
		$nowtime=(int)time()/86400;
                $bought_time=(int)$bought->datetime/86400
                
		while(($nowtime - $bought_time) % $shop->reset() == 0){
		$nowtime=$nowtime+1;
		}
                $reset_time=$nowtime;
		//$reset_time=(($nowtime-$bought->datetime)/86400) % $shop->reset();
               if ($reset_time<=0){
	...
	public function node($request, $response, $args)
	{
		...
		foreach ($nodes as $node) {
			...
			if (Config::get('show_stream_media')) {
				...
				if (Config::get('streaming_media_unlock_multiplexing') != null) {
					$value = Config::get('streaming_media_unlock_multiplexing');
	...
	public function ticket_view($request, $response, $args)
	{
		...
		$ticketset = Ticket::where('id', $id)
			->orWhere('rootid', '=', $id)
			->orderBy('datetime', 'desc')
			->paginate(5, ['*'], 'page', $pageNum);
